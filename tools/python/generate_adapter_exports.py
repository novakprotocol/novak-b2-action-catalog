...
NUMBER = re.compile(r"-Number\s+([0-9]+)")
LAYER = re.compile(r"-LayerId\s+'([^']+)'")
AUDIENCE = re.compile(r"-Audience\s+'([^']+)'")
...
def contract_from(node: dict[str, Any], rel: str) -> dict[str, Any]:
    ...
    number = node.get("action_number") or node.get("number") or meta.get("action_number")
    if not action_id or number is None:
        fail(f"missing action id or number for {rel}")
    parts = rel.split("/")
    title = node.get("title") or meta.get("title") or title_from_id(str(action_id))
    ...
    return {
        ...
        "subdomain": str(node.get("subdomain") or meta.get("layer_id") or (parts[2] if len(parts) > 2 else "unknown")),
        ...
        "adapter_hints": {"ai": "explain_select_recommend_only_no_execution_approval", "human": "runbook_preview_evidence_only", "ansible": "preview_metadata_only", "awx": "job_template_preview_no_credential", "aws_ssm": "document_preview_echo_only", "servicenow": "import_set_preview_metadata_only", "cmdb": "candidate_mapping_preview_metadata_only"},
...
def load_contracts(index_path: Path) -> list[dict[str, Any]]:
    ...
    out = sorted(by_id.values(), key=lambda x: (x["action_number"], x["action_id"]))
    if not out:
...
def emit(sample: list[dict[str, Any]], source_count: int, out_dir: Path) -> None:
    ...
    manifest = {"schema_version": "adapter-manifest.v1", "generated_utc": now, "source_index": "catalog/generated/action-index.json", "source_action_count": source_count, "sample_count": len(sample), "bounded_preview": True, "boundary": {"safety_class": "evidence_only", "mutation_allowed": False, "live_platform_calls_allowed": False, "secrets_allowed": False, "raw_target_lists_allowed": False, "network_scan_allowed": False}, "adapters": adapters, "sample_actions": [{"action_id": c["action_id"], "action_number": c["action_number"], "script_path": c["script_path"], "script_sha256": c["script_sha256"]} for c in sample]}
