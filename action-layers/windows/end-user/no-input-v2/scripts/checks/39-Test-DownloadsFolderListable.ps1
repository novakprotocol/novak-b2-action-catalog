# ENDUSER_TEST_DOWNLOADS_FOLDER_LISTABLE_V1
...
$ctx = New-ItopsEvidenceContext -ActionId "ENDUSER_TEST_DOWNLOADS_FOLDER_LISTABLE_V1" -EvidenceRoot $EvidenceRoot
...
Invoke-ItopsSafe -Context $ctx -ScriptBlock {
    ...
    Write-ItopsResult -Context $ctx -Result $status -Message "Checked Downloads folder visibility/listability without recording path." -Data @{ exists=$exists; can_list=$canList; path_redacted=$true } -ExitCode $(if ($status -eq "PASS") {0} elseif ($status -eq "WARN") {2} else {1})
